...
export default function Seller() {
  ...
  async function fetchOrders() {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        products ( name, price ),
        usuarios!orders_buyer_id_fkey ( nombre, email )
      `)
  ...
  const totalVentas = orders.reduce((sum, o) => sum + Number(o.amount), 0)
        ...
        {tab === 'orders' && (
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
              <StatCard label="Total ventas" value={orders.length} icon="🧾" />
              <StatCard label="Ingresos totales" value={`$${totalVentas.toFixed(2)}`} icon="💰" />
              <StatCard
                label="Pendientes"
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Producto</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">Comprador</th>
                    ...
                    {orders.map(order => (
                      <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-500">
                          {order.usuarios?.nombre ?? order.usuarios?.email ?? '—'}
                        </td>
